class mazeGenerator {
  ...
  async generateRecursiveDivision() {
    ...
    function divide(grid, x, y, width, height, orientation) {
      ...
      // location of hole
      let holeX = wallX + (orientation ? rand(width / 2) : 0);
      let holeY = wallY + (orientation ? 0 : rand(height / 2));
      let holeX2 = wallX + (orientation ? rand(width - 1) : 0);
      let holeY2 = wallY + (orientation ? 0 : rand(height - 1));
      let holeX3 = wallX + (orientation ? rand(width - 1) : 0);
      let holeY3 = wallY + (orientation ? 0 : rand(height - 1));
      // direction of wall
      ...
      while (wallLength--) {
        if (
          ...
          (holeX3 != wallX || holeY3 != wallY)
        ) {
          if (grid[wallX][wallY].type === "empty")
      ...
      let newX = x;
      let newY = y;
      let newWidth = orientation ? width : wallX - x;
      let newHeight = orientation ? wallY - y : height;
      divide(
        grid,
        newX,
        newY,
    ...
    for (let i = 0, k = this.nodeArray.length - 1; i <= k; i++, k--) {
      for (
        ...
        if (chance % 2 && this.nodeArray[i][j].type === "empty") {
          let secondChance = Math.floor(Math.random() * 100);
          if (secondChance % 2) {
            this.nodeArray[i][j].type = "weighted";
            await this.update(this.nodeArray[i][j], 0.1);
          }
        }
        if (chance % 2 && this.nodeArray[k][l].type === "empty") {
          let secondChance = Math.floor(Math.random() * 100);
          if (secondChance % 2) {
            this.nodeArray[k][l].type = "weighted";
            await this.update(this.nodeArray[k][l], 0.1);
          }
        }
      }
  ...
  async generatreWeigtedRecursiveDivision() {
    this.clear();
    function rand(value) {
      return Math.floor(Math.random() * value);
    }
